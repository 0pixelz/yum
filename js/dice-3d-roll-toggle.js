...
(function () {
  ...
  function install() {
    ...
    function rollDice3D() {
      ...
      // Power-up multiplayer rolls locally (client-authoritative), and the local
      // 3D path works for the first throw — but the in-overlay "Roll again"
      // misbehaves in live multiplayer in a way that doesn't reproduce in
      // power-up vs-bot (identical code) or headlessly. Until that's diagnosed on
      // a real device, fall back to the reliable 2D roll here. 3D still runs in
      // power-up vs-bot and in normal multiplayer.
